...
//只有1个分类的 数据格式要处理一下
 if(array_key_exists('id',$category_top)) {
 	$category_top_tmp = array();
 	$category_top_tmp[]= $category_top;
 	$category_top = array();
 	$category_top = $category_top_tmp;
 }
 
 //根据子分类得到父分类的ID
 $video_info['top_category'] = '';
 foreach($category_top as $category_top_value) {
 	if(array_key_exists('id',$category_top_value['sub-category'])) {
	 	if($category_top_value['sub-category']['id'] == $video_info['category']) {
	 		 $video_info['top_category'] = $category_top_value['id'];
	 	}
 	 } else {
 	 	foreach($category_top_value['sub-category'] as $category_sub_value) {
 	 		if($category_sub_value['id'] ==$video_info['category']) {
 	 			 $video_info['top_category'] = $category_top_value['id'];
 	 		}
 	 	}
 	 }
